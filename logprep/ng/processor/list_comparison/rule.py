...
.. autoclass:: logprep.ng.processor.list_comparison.rule.ListComparisonRule.Config
   :members:
   :undoc-members:
   :inherited-members:
   :noindex:
...
.. datatemplate:import-module:: tests.unit.ng.processor.list_comparison.test_list_comparison
...
class ListComparisonRule(FieldManagerRule):
    ...
    async def init_list_comparison(
        self,
        callback_tag: str,
        ...
        if not base_path.startswith("http"):
            await self._init_list_comparison_from_local_file(base_path, list_paths, list_names)
        else:
            await self._init_list_comparison_from_http(base_path, list_paths, list_names)

    async def _add_static_compare_set(self, name: ListName, path: str) -> None:
        compare_set = _StaticCompareSet(name=name, content=set())
        self._static_sets.append(compare_set)
        await self._load_and_refresh_uri(compare_set, path)
    ...
    async def _update_compare_sets_via_http(self, getter: Getter, compare_set: _CompareSet) -> None:
        try:
            content = await self._get_list_contents_from_getter(getter)
    ...
    async def _init_list_comparison_from_local_file(
        ...
        for list_name, list_path in zip(list_names, absolute_paths):
            content = await self._get_list_contents_from_getter(
                GetterFactory.from_string(list_path)
            )
            self._static_sets.append(_StaticCompareSet(name=list_name, content=content))

        self.compare_set_names = list_names

    async def _init_list_comparison_from_http(
        self, base_path: str, list_paths: Sequence[str], list_names: Sequence[str] | None
    ) -> None:
        ...
        for name, list_path in zip(list_names, list_paths):
            ...
            if dynamic_identifiers:
                ...
                await self._add_static_compare_set(name=name, path=full_path_with_env)
    ...
    async def _get_list_contents_from_getter(self, getter: Getter) -> set:
        raw_list = await getter.get_list(content_field=self._config.content_field)
        return {
            elem
    ...
    async def _load_and_refresh_uri(self, compare_set: _CompareSet, uri: str) -> None:
