...
class ConfluentKafkaInput(Input):
    ...
    async def _assign_callback(self, _, topic_partitions: list[TopicPartition]) -> None:
        for topic_partition in topic_partitions:
    ...
    async def _revoke_callback(self, _: Any, topic_partitions: list[TopicPartition]) -> None:
    ...
    async def _lost_callback(self, topic_partitions: list[TopicPartition]) -> None:
        for topic_partition in topic_partitions:
            self.metrics.number_of_warnings += 1
            member_id = await self._get_memberid()
            logger.warning(
