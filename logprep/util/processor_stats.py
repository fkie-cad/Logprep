...
class ProcessorStats:
    ...
    def update_per_rule(self, idx: int, processing_time: float, rule):
...
class StatusTracker:
    ...
    def __init__(self, shared_dict: dict, print_period: float, status_logger: Logger, lock: Lock):
        ...
        self.aggr_data = {'errors': 0, 'warnings': 0, 'processed': 0, 'error_types': dict(), 'warning_types': dict()}
        self._lock = lock
    ...
    def add_warnings(self, error: BaseException, processor: BaseProcessor):
        ...
        warning_types[str(error)] = warning_types[str(error)] + 1 if str(error) in warning_types else 1

    def add_errors(self, error: BaseException, processor: BaseProcessor):
        self.aggr_data['errors'] += 1
        processor.ps.aggr_data['errors'] += 1
        warning_types = self.aggr_data['error_types']
        warning_types[str(error)] = warning_types[str(error)] + 1 if str(error) in warning_types else 1
    ...
    def _print_aggregated_data_if_all_processes_have_been_written_to_shared_process_dict(self):
        with self._lock:
            if not any([value is None for value in self._shared_dict.values()]):
                ...
                self._add_derivative_data(aggregated_data)
                filtered_data = self._get_filtered_stats(aggregated_data)
                filtered_data['timestamp'] = datetime.now().isoformat()
                ordered_data = self._get_sorted_output_dict(filtered_data)
    ...
    def _add_derivative_data(aggr_data: dict):
        for name, v1 in aggr_data.items():
            if isinstance(v1, dict):
                if not name.startswith('Multiprocessing') and name not in ('error_types', 'warning_types'):
                    ...
                    times_per_idx = np.divide(times_per_idx, matches_per_idx, out=np.zeros_like(times_per_idx),
    ...
    def _get_aggregated_data_from_pipeline(self) -> dict:
        ...
        for process in processes:
            ...
            self._add_relevant_values_to_multiprocessing_pipelines(aggregated_data, process)
            ...
            self._aggregate_error_types(aggregated_data, process)

        return aggregated_data

    def _aggregate_processor_specific(self, aggregated_data: dict, excluded_keys: tuple, process: dict):
    ...
    def _get_filtered_stats(self, aggr_data: dict) -> dict:
        filtered_dict = deepcopy(aggr_data)
        self._remove_numpy_arrays(aggr_data, filtered_dict)
        return filtered_dict

    @staticmethod
    def _remove_numpy_arrays(aggr_data: dict, filtered_dict: dict):
        _dict = filtered_dict
        for k1, v1 in aggr_data.items():
            if isinstance(v1, dict):
