...
class FieldManager(Processor):
    ...
    def _apply_single_target_processing(
        ...
        source_field_values = [
            value for value in source_field_values if self.is_processable_value(value)
        ]
    ...
    def _flatten_values(
        ...
        for field_value in flat_source_fields:
            if deduplicate and field_value in present_keys:
                continue
            present_keys.append(field_value)
    ...
    @staticmethod
    def _get_field_values(event: dict[str, FieldValue], source: Iterable[str]) -> list[FieldValue]:
        return [get_dotted_field_value(event, source_field) for source_field in source]

    def _get_missing_fields_error(
        self, source_fields: Iterable[FieldValue], field_values: Iterable[FieldValue]
    ):
    ...
    @staticmethod
    def _get_flatten_source_fields(source_fields_values: Iterable[FieldValue]) -> list[FieldValue]:
        flat_source_fields = []
    ...
    def _filter_missing_fields(
        self, source_field_values: list[FieldValue], targets: list[str]
    ) -> tuple[list[FieldValue], list[str]]:
        if all(self.is_processable_value(value) for value in source_field_values):
        ...
        for value, target in zip(source_field_values, targets):
            if self.is_processable_value(value):
                values.append(value)
    ...
    def is_processable_value(self, value: FieldValue) -> bool:
        """Return whether a source value should be passed to ``transform_values``.

        Subclasses can override this hook to reject otherwise present values. For mappings,
        the corresponding target is filtered together with each rejected source value.
        """
