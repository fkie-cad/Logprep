...
extraction component. During operation, features learned during the training pahse will be
...
and vectorizer for rule attrbution and feature extraction are held by :code:`RuleAttributor` instances.
...
attribution results can be retrieved from the cache in a shorter amount of time. The :code:`max_cache_entries` 
configuration parameter determines the size of the internal cache. 
...
file is given by the :code:`models_path` confiugration parameter. An example of a configuration of the 
...
class Amides(Processor):
    ...
    class ProcessorMetrics(Processor.ProcessorMetrics):
        ...
        """Mean processing time of command-lines classified by the rule attributor."""
