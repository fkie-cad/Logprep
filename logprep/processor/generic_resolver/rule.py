...
class GenericResolverRule(Rule):
    ...
    def __eq__(self, other: 'GenericResolverRule') -> bool:
        return (other.filter == self._filter) and (
                self._field_mapping == other.field_mapping) and (
                self._resolve_list == other.resolve_list) and (
    ...
    def _check_if_valid(rule: dict):
        ...
        for field in ('field_mapping',):
            if not isinstance(generic_resolver_cfg[field], dict):
                raise InvalidGenericResolverDefinition('"{}" value "{}" is not a string!'.format(
