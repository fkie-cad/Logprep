        ...
        if add_paths is not None:
            ...
            if only_first_existing:
                if existing_files:
                    ...
                    raise InvalidGenericAdderDefinition(f'At least one of the following files must '
                                                        f'exist: \'{missing_files}\'')
            else:
                if missing_files:
                    raise InvalidGenericAdderDefinition(f'The following required files do '
                                                        f'not exist: \'{missing_files}\'')

            for add_path in existing_files:
                with open(add_path, 'r') as add_file:
                    add_dict = yaml.load(add_file)
                    if (isinstance(add_dict, dict) and
                            all(isinstance(value, str) for value in add_dict.values())):
                        self._add_from_file.update(add_dict)
                    else:
                        error_msg = (f'Additions file \'{add_path}\' must be a dictionary '
    ...
    def _check_if_valid(rule: dict):
        ...
        for field in ('add',):
            if generic_adder_cfg.get(field) and not isinstance(generic_adder_cfg[field], dict):
                raise InvalidGenericAdderDefinition('"{}" value "{}" is not a dict!'.format(
                                                field, generic_adder_cfg[field]))
        for field in ('add_from_file',):
            if generic_adder_cfg.get(field):
                error_msg = (f'"{field}" value "{generic_adder_cfg[field]}" is not a string or a'
                             f' list of strings!')
