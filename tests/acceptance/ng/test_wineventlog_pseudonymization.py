...
def test_events_pseudonymized_correctly(tmp_path, config: Configuration):
    ...
    output_bytes = Path(config.output["jsonl"]["output_file"]).read_bytes()
    extra_output_bytes = Path(config.output["jsonl"]["output_file_custom"]).read_bytes()
    if Path(config.error_output["error"]["output_file"]).exists():
        error_output_bytes = Path(config.error_output["error"]["output_file"]).read_bytes()
    else:
        error_output_bytes = b""

    decoder = msgspec.json.Decoder()

    output = decoder.decode_lines(output_bytes)
    extra_output = decoder.decode_lines(extra_output_bytes)
    error_output = decoder.decode_lines(error_output_bytes)
    ...
    assert set(map(frozenset, output)) == set(map(frozenset, expected_logprep_outputs))
