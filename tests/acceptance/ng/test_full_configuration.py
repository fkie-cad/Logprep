...
from tests.acceptance.ng.util import (
...
async def test_start_of_logprep_with_full_configuration_http():
    pipeline = get_full_pipeline(exclude=["normalizer", "geoip_enricher"])
    config = get_default_logprep_config(pipeline, with_hmac=False)
    config.output.update({"kafka": {"type": "dummy_output", "default": False}})
    endpoint = "http://localhost:32000"
    config = await convert_to_http_config(config, endpoint)
...
def test_start_of_logprep_from_http_with_templated_url_and_config(unused_tcp_port, tmp_path):
    config_path = tmp_path / "generated_config.yml"
...
""",
    ...
    env = {
        "LOGPREP_API_ENDPOINT": f"http://localhost:{unused_tcp_port}",
        "LOGPREP_VERSION": "1",
...
async def test_config_loglevel_error_hides_info(unused_tcp_port):
    pipeline = get_full_pipeline(exclude=["normalizer", "geoip_enricher"])
    config = get_default_logprep_config(pipeline, with_hmac=False)
    config.output.update({"kafka": {"type": "dummy_output", "default": False}})
    ...
    endpoint = f"http://localhost:{unused_tcp_port}"
    config = await convert_to_http_config(config, endpoint)
    config_path = Path("generated_config.yml")
    config_path.write_text(config.as_yaml(), encoding="utf-8")

    with HTTPServerForTesting.run_in_thread(port=unused_tcp_port):
        with run_logprep(f"{endpoint}/{config_path.name}") as proc:
            while True:
                output = proc.stdout.readline().decode("utf8")
...
async def test_config_loglevel_info_shows_info(unused_tcp_port):
    pipeline = get_full_pipeline(exclude=["normalizer", "geoip_enricher"])
    config = get_default_logprep_config(pipeline, with_hmac=False)
    config.output.update({"kafka": {"type": "dummy_output", "default": False}})
