...
from tests.acceptance.ng.util import run_logprep, wait_for_output
...
async def test_error_output_for_critical_input_error_with_missing_hmac_target_field(
    ...
    with run_logprep(config_path) as proc:
        ...
        wait_for_output(
            proc,
            "Couldn't find the hmac target field",
            forbidden_outputs=("Invalid", "Exception"),
        )
    start = time.time()
...
def test_error_output_errors_are_logged_if_error_output_has_an_error(
    ...
    config.input = {
        "dummy": {
            "type": "dummy_input",
            "documents": [{"something": "yeah"}, "ErrorEvent"],
            "exhaustable": False,
        }
    }
    ...
    with run_logprep(config_path) as proc:
        wait_for_output(
            proc,
            r"failed to store .* error events in the error output",
            test_timeout=1000,
            forbidden_outputs=(),
        )
