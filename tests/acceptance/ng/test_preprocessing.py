...
class TestVersionInfoTargetField:
    async def test_preprocessor_adds_version_information(
        self, tmp_path: Path, config: Configuration
    ):
        ...
        test_outputs = await get_test_outputs(str(config_path))
        test_output = test_outputs["jsonl"]
