...
async def test_events_labeled_correctly(
    ...
    out = await get_test_outputs(str(config_path))
    assert out["jsonl"], "should not be empty"
    store_latest_test_output(expected_output, out["jsonl"])
    expected_output = parse_jsonl(expected_output_path)
    result = get_difference(out["jsonl"], expected_output)
    assert (
