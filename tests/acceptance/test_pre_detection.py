...
def test_events_pre_detected_correctly(
    ...
    diff = DeepDiff(
        expected_output_event, logprep_output[0], exclude_paths="root['pre_detection_id']"
    )
    ...
    if expected_extra_output is not None:
        ...
        for expected_extra_out in expected_extra_output:
            ...
            for logprep_extra_out in logprep_extra_output:
                exclude_regex_path = re.compile(
                    r"root\['pre_detector_topic'\]\['pre_detection_id'\]"
                )
                diff = DeepDiff(
                    expected_extra_out, logprep_extra_out, exclude_regex_paths=exclude_regex_path
                )
                if not diff:
