...
def test_events_pre_detected_correctly(tmp_path, config):
    ...
    assert result_detections['difference'][0] == result_detections['difference'][1],\
    ...
    result_detections = get_difference_detections(test_output_detections, expected_output_detections)
    assert result_detections['difference'][0] == result_detections['difference'][1],\
        'Missmatch in event at line {}!'.format(result_detections['event_line_no'])
