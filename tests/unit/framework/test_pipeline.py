...
class TestPipeline(ConfigurationForTests):
    ...
    def test_retrieve_and_process_data_raises_exceptions_that_occur_while_retrieving_data(self):
        pipeline = self.create_pipeline([ValueError],
                                        ['donothing'])
    ...
    def test_logs_source_disconnected_error_as_warning(self):
        ...
        with AssertEmitsLogMessage(self.log_handler, WARNING,
                                   prefix='Lost or failed to establish connection to '):
            pipeline.run()
    ...
    def test_fatal_input_error_is_logged_and_pipeline_shuts_down(self):
        pipeline = self.create_pipeline([FatalInputError, {'event': 'never processed'}],
                                        ['donothing'])
    ...
    def test_input_warning_error_is_logged_but_processing_continues(self):
        pipeline = self.create_pipeline([{'order': 0}, WarningInputError, {'order': 1}],
                                        ['donothing'])

        with AssertEmitsLogMessage(self.log_handler, WARNING,
                                   contains='An error occurred for input dummy'):
            pipeline.run()
    ...
    def test_output_warning_error_is_logged_but_processing_continues(self):
        pipeline = self.create_pipeline([{'order': 0}, {'order': 1}], ['donothing'],
                                        [WarningOutputError])

        with AssertEmitsLogMessage(self.log_handler, WARNING,
                                   contains='An error occurred for output dummy'):
            pipeline.run()
    ...
    def test_processor_warning_error_is_logged_but_processing_continues(self):
        ...
        pipeline_config = [{'before': {'type': 'donothing'}},
                           {'failing': {'type': 'donothing',
                                        'errors': [ProcessorWarningMockError]}},
