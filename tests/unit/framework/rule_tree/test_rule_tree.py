...
class TestRuleTree:
    ...
    def test_add_rule(self):
        rt = RuleTree()
        rule = PreDetectorRule._create_from_dict({"filter": "winlog: 123",
    ...
    def test_get_rule_id(self):
        ...
        rule2 = PreDetectorRule._create_from_dict({"filter": "winlog: 123 AND xfoo: bar",
                                                   'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                    'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule2)
        assert rt.get_rule_id(rule) == 0
    ...
    def test_match_complex_case(self):
        rt = RuleTree()
        rule = PreDetectorRule._create_from_dict({"filter": "winlog: 123 AND test: (Good OR Okay OR Bad) OR foo: bar",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)
    ...
    def test_match_event_matches_multiple_rules(self):
        rt = RuleTree()
        rule = PreDetectorRule._create_from_dict({"filter": "winlog: 123 AND test: (Good OR Okay OR Bad)",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)

        rule2 = PreDetectorRule._create_from_dict({"filter": "foo: bar",
                                                   'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                    'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule2)

        assert rt.get_matching_rules({'winlog': '123', 'test': 'Good', 'foo': 'bar'}) == [rule, rule2]

    def test_match_exists_filter_is_subfield(self):
        rt = RuleTree()
        rule = PreDetectorRule._create_from_dict({"filter": "foo.bar: 123",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)
        assert rt.get_matching_rules({'foo': {'bar': '123'}}) == [rule]

        rule = PreDetectorRule._create_from_dict({"filter": "foo.bar.test: 123",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)
        assert rt.get_matching_rules({'foo': {'bar': {'test': '123'}}}) == [rule]

        rule = PreDetectorRule._create_from_dict({"filter": "abc: DEF AND foo.bar.test: 567",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)
    ...
    def test_match_including_tags(self):
        ...
        assert rt.get_matching_rules({"winlog": "123", "test": "Okay", "source": {"windows": "foo"}})

    def test_match_with_subrules(self):
        rt = RuleTree()
        rule = PreDetectorRule._create_from_dict({"filter": "EventID: 1 AND winlog: 123",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)

        subrule = PreDetectorRule._create_from_dict({"filter": "EventID: 1",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(subrule)
    ...
    def test_get_size(self):
        ...
        rule = PreDetectorRule._create_from_dict({"filter": "winlog: 123 AND xfoo: foo",
                                                  'pre_detector': {'id': 1, 'title': '1', 'severity': '0',
                                                                   'case_condition': 'directly', 'mitre': []}})
        rt.add_rule(rule)
