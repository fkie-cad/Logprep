...
class TestJsonlOutputOutput(BaseOutputTestCase):
    ...
    async def test_store_custom_appends_document_to_variable(self):
        await self.object.store_custom(self.event, target="whatever")
        assert len(self.object.events) == 1
        assert self.object.events[0] == {"whatever": self.event.data}

    async def test_store_maintains_order_of_documents(self):
        for i in range(0, 3):
            await self.object.store(LogEvent({"order": i}, original=b""))
    ...
    async def test_write_document_to_file_on_store(self, _):
        await self.object.store(self.event)
        self.object._write_json.assert_called_with(self.CONFIG["output_file"], self.event.data)

    @mock.patch("logprep.ng.connector.jsonl.output.JsonlOutput._write_json")
    async def test_write_document_to_file_on_store_custom(self, _):
        await self.object.store_custom(self.event, target="whatever")
    ...
    async def test_write_multiple_documents_to_file_on_store(self, _):
        await self.object.store(LogEvent(self.event.data, original=b""))
        await self.object.store(LogEvent(self.event.data, original=b""))
    ...
    async def test_write_json_writes_to_file(self, mock_open):
        JsonlOutput._write_json("the/file/path", self.event.data)
        mock_open.assert_called_with("the/file/path", "a+", encoding="utf8")

    @mock.patch("builtins.open")
    async def test_setup_creates_single_file_if_only_output_file(self, mock_open):
    ...
    async def test_store_counts_processed_events(self, _):  # pylint: disable=arguments-differ
        ...
        await self.object.store(LogEvent({"message": "my event message"}, original=b""))
