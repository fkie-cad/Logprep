...
class TestConfluentKafkaOutput(BaseOutputTestCase):
    ...
    async def test_producer_property_instantiates_kafka_producer(self):
    ...
    @mock.patch("logprep.ng.connector.confluent_kafka.output.Producer")
    async def test_shut_down_calls_producer_flush(self, _):
        kafka_producer = self.object._producer
        self.object.shut_down()
        kafka_producer.flush.assert_called()

    @mock.patch("logprep.ng.connector.confluent_kafka.output.Producer")
    async def test_raises_critical_output_on_any_exception(self, _):
    ...
    async def test_setup_raises_fatal_output_error_on_invalid_config(self):
    ...
    async def test_health_returns_true_if_no_error(self):
        self.object._admin = mock.MagicMock()
    ...
    async def test_health_returns_false_if_topic_not_present(self):
        self.object._admin = mock.MagicMock()
        metadata = mock.MagicMock()
    ...
    async def test_on_delivery_successful(self, caplog):
    ...
    async def test_on_delivery_unsuccessful(self, caplog):
