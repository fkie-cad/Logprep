...
from logprep.ng.processor.list_comparison.rule import (
    ListComparisonRule,
    _StaticCompareSet,
)
...
class TestListComparisonRule:
    ...
    async def test_compare_sets_not_empty_for_valid_rule_def_after_init_list_comparison(
        self,
        rule_definition,
    ):
        ...
        await rule.init_list_comparison(
            "test_owner",
            "tests/testdata/unit/list_comparison/rules",
        )

        compare_sets = {name: content async for name, content in rule.iter_compare_sets({})}

        assert compare_sets == {"user_list.txt": {"Franz"}}
        assert list(rule.compare_set_names) == ["user_list.txt"]

    async def test_init_list_comparison_raises_if_no_base_path_is_configured(self):
        rule_definition = {
            "filter": "user",
            "list_comparison": {
                "source_fields": ["user"],
        ...
        with pytest.raises(
            ...
            await rule.init_list_comparison("test_owner")
    ...
    async def test_expects_refreshable_getter(self, url, will_fail):
        rule_definition = {
            "filter": "user",
            "list_comparison": {
                "source_fields": ["user"],
        ...
        rule = typing.cast(
            ListComparisonRule,
            ListComparisonRule.create_from_dict(rule_definition),
        )
        compare_set = _StaticCompareSet(
            name="user_list.txt",
            content=set(),
        )

        with mock.patch(
            "logprep.ng.processor.list_comparison.rule."
            "ListComparisonRule._update_compare_sets_via_http",
            new=mock.AsyncMock(),
        ):
            if will_fail:
                with pytest.raises(
                    TypeError,
                    match=f"The target {url} must be a url",
                ):
                    await rule._load_and_refresh_uri(compare_set, url)
            else:
                await rule._load_and_refresh_uri(compare_set, url)
            ...
            pytest.param(
                {
                    ...
                    "target_field": "user_results",
                    "list_file_paths": [
                        "some/nice/path/list.txt",
                        "some/cool/path/list.txt",
                    ],
                },
    ...
    async def test_rule_config_is_validated(
        self,
        list_comparison_config,
        error_message,
    ):
        rule_dict = {
            "filter": "user",
            "list_comparison": list_comparison_config,
        }
