...
class TestNetworkComparisonRule:
    ...
    async def test_compare_sets_not_empty_for_valid_rule_def_after_init_list_comparison(
        ...
        await rule.init_list_comparison(
            "test_owner",
            "tests/testdata/unit/network_comparison/rules",
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "Compare empty file",
                ["../lists/empty_list.txt"],
                {"empty_list.txt": set()},
            ),
        ],
    )
    async def test_compare_set_with_network_and_string_loads_correctly(
        self,
        testcase,
        lists,
        expected,
    ...
    async def test_rule_load_fails_if_network_invalid(self):
        rule_definition = {
            "filter": "ip",
            "network_comparison": {
                "source_fields": ["ip"],
        ...
        with pytest.raises(
            ValueError,
            match="'invalid_network' does not appear to be an IPv4 or IPv6 network",
        ):
            await rule.init_list_comparison(
                "test_owner",
                "tests/testdata/unit/network_comparison/rules",
