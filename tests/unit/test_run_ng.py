...
class TestRunLogprepNGCli:
    ...
    async def test_cli_commands_with_configs(self, command: str, target: str):
    ...
    async def test_cli_invokes_default_config_location(self, command, caplog):
        result = self.cli_runner.invoke(cli, [*command])
        assert result.exit_code != 0
        assert "does not exist: /etc/logprep/pipeline.yml" in caplog.text

    async def test_cli_run_starts_runner_with_config(self):
        config_file_path = ("tests/testdata/config/config-ng.yml",)
        expected_config = await Configuration.from_sources(config_file_path)
    ...
    async def test_cli_run_starts_runner_with_multiple_configs(self):
        config_file_path = (
    ...
    async def test_exits_after_getter_error_for_not_existing_protocol(self, caplog):
    ...
    async def test_version_arg_prints_logprep_version(self):
    ...
    async def test_run_version_arg_prints_logprep_version_with_config_version(self):
    ...
    async def test_run_version_arg_prints_logprep_version_without_config_value(self):
    ...
    @responses.activate
    async def test_run_version_arg_prints_with_http_config(self):
    ...
    async def test_run_no_config_error_is_printed_if_given_config_file_does_not_exist(self, caplog):
    ...
    async def test_main_calls_runner_stop_on_any_exception(self, mock_runner):
    ...
    async def test_logprep_exits_on_invalid_configuration(self):
    ...
    async def test_logprep_exits_on_any_exception_during_verify(self):
        with mock.patch("logprep.ng.util.configuration.Configuration._verify") as mock_verify:
    ...
    async def test_logprep_exits_on_request_exception(self):
        with mock.patch("logprep.util.getter.HttpGetter._get_raw") as mock_get_raw:
            mock_get_raw.side_effect = requests.RequestException("connection refused")
            config_path = "http://localhost/does-not-exists"
            result = self.cli_runner.invoke(cli, ["run", config_path])
            assert result.exit_code == EXITCODES.CONFIGURATION_ERROR.value

    @mock.patch("logging.Logger.info")
    async def test_run_logprep_logs_log_level(self, mock_info):
        config = await Configuration.from_sources(("tests/testdata/config/config-ng.yml",))
