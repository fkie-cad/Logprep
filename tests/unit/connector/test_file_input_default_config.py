...
from tests.unit.connector.base import BaseConnectorTestCase, BaseInputTestCase
...
class TestFileInput(BaseInputTestCase):
    ...
    def test_get_unique_logs_after_rotating_filechange_detected_with_filesize_smaller_256(self):
        ...
        while not self.object._messages.empty():
            self.object._messages.get(timeout=0.001)
        write_empty_file(testfile_location)
        wait_for_interval(check_interval)
        empty_file_fingerprint_size = self.object._fileinfo_util.get_fingerprint_size(self.object._config.documents_path)
        # append first line to empty file with less than 256 byte in total
        append_file(testfile_location, test_rotated_log_data_less_256)
        wait_for_interval(check_interval)
        first_small_file_fingerprint_size =  self.object._fileinfo_util.get_fingerprint_size(self.object._config.documents_path)
        # append second line to empty file with less than 256 byte in total
        append_file(testfile_location, test_rotated_log_data_less_256)
        wait_for_interval(check_interval)
        second_small_file_fingerprint_size = self.object._fileinfo_util.get_fingerprint_size(self.object._config.documents_path)
        while not self.object._messages.empty():
            queued_logs.append(self.object._messages.get(timeout=0.001))
