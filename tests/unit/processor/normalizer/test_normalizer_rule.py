...
class TestNormalizerRule:
    def test_rules_are_equal(self, specific_rule_definition):
        rule1 = NormalizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                               specific_rule_definition['normalize'])

        rule2 = NormalizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                               specific_rule_definition['normalize'])

        assert rule1 == rule2

    def test_rules_are_not_equal(self, specific_rule_definition):
        rule = NormalizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                              specific_rule_definition['normalize'])

        rule_diff_substi = NormalizerRule(LuceneFilter.create(
            specific_rule_definition['filter']), specific_rule_definition['normalize'])

        rule_diff_filter = NormalizerRule(LuceneFilter.create(
            specific_rule_definition['filter']), specific_rule_definition['normalize'])
