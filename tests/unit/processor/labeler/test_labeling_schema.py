...
from logprep.processor.base.exceptions import KeyDoesnotExistInSchemaError, ValueDoesnotExistInSchemaError
...
class SchemasForTests:
            ...
            'operatingsystem': {
                ...
                'unixlike': {
                    'linux': {
                        'description': 'Linux',
                        'android': {
                            'description': 'Android operating system for mobile devices'
                        }
                    }
                }
            }
    ...
    minimal_schema = {
        'reporter': {
            'category': 'The event\'s reporter',
            'windows': {
                'description': 'the description may actually be empty'
            }
        }
    }
    ...
    invalid_schema_category_without_category_field = {
        ...
        'invalid': {
            'valid label': {
                'description': 'this label is valid, the category is not'
            }
        }
    }
    ...
    invalid_schema_with_duplicate_label_in_same_category = {
            'reporter': {
                ...
                'valid': {
                    'windows': {
                        'description': 'invalid, duplicate label'
                    }
                }
            },
        }
...
class TestLabelingSchemaCreateFromFile(SchemasForTests):
    ...
    def test_create_schema_fails_if_path_points_to_directory(self):
        with raises(InvalidLabelingSchemaFileError, match=r'Not a valid schema file: \[Errno 21\] Is a directory: \'.*\'.'):
    ...
    def test_create_schema_fails_is_file_not_valid_json(self):
        with JsonTempFile({}) as schema_file:
            with raises(InvalidLabelingSchemaFileError, match='Not a valid schema file: JSON decoder error: .*: ".*".'):
    ...
    def test_fails_if_schema_contains_description_with_non_string_value(self):
        ...
        for non_string in [123, 456.789, None]:
            ...
            self.assert_fails_with_expected_message(schema,
    ...
    def test_rejects_schema_that_has_category_without_category_field(self):
        self.assert_fails_with_expected_message(self.invalid_schema_category_without_category_field,
    ...
    def test_rejects_schema_that_has_non_string_category_field(self):
        self.assert_fails_with_expected_message(self.invalid_schema_category_with_non_string_category_field,
    ...
    def test_rejects_schema_that_contains_non_description_leaf(self):
        self.assert_fails_with_expected_message(self.invalid_schema_category_with_non_description_leaf,
    ...
    def test_rejects_schema_with_duplicate_label_in_same_category(self):
        self.assert_fails_with_expected_message(self.invalid_schema_with_duplicate_label_in_same_category,
...
class TestLabelingSchema(SchemasForTests):
    ...
    def test_fails_if_schema_contains_description_with_non_string_value(self):
        ...
        for non_string in [123, 456.789, None]:
            schema['reporter']['key']['description'] = non_string
            with raises(LabelWithoutDesciptionInSchemaError, match='Label "key" does not have a valid description'):
    ...
    def test_rejects_schema_that_contains_non_description_leaf(self):
        with raises(NonDescriptionLeafError, match='Invalid schema definition: "key" is a leaf but not a description'):
            self.schema.ingest_schema(self.invalid_schema_category_with_non_description_leaf)

    def test_rejects_schema_with_duplicate_label_in_same_category(self):
        with raises(DuplicateLabelInCategoryError, match='Invalid schema definition: Category "reporter" contains label "windows" more than once'):
    ...
    def test_top_level_entries_must_not_have_description(self):
        with raises(CategoryMustNotContainDescriptionFieldError, match='Category "invalid" must not have a description field'):
    ...
    def test_does_not_accept_labels_not_in_schema(self):
        ...
        for invalid_label in ['not a defined label', 'linux', 'macos']:
            with raises(ValueDoesnotExistInSchemaError, match='Invalid value \'.*\' for key \'reporter\''):
    ...
    def test_toplevel_entry_REFERENCES_does_not_show_up_in_schema(self):
        ...
        schema['REFERENCES'] = {
            ...
            'key': {'description': 'This should not be a valid label without being referenced elsewhere.'}
        }
    ...
    def test_can_use_json_references_in_json_file_to_simplify_document(self):
        ...
        schema['REFERENCES'] = {
            'category': 'This "category" may be used for storing reference objects',
            'key': {
                'label': {'description': 'This should not be a valid label without being referenced elsewhere.'}
            }
        }
