...
from tests.testdata.metadata import (path_to_single_rule, path_to_invalid_rules, path_to_schema, path_to_schema2,
                                     path_to_testdata, path_to_rules2)
...
class TestLabelerFactory:
    ...
    def test_create_fails_when_config_does_not_contain_rules(self):
        ...
        with raises(RulesDefinitionMissingError, match='The labeler configuration must contain at least one path to a rules directory.'):
    ...
    def test_create_fails_when_config_does_not_contain_schema(self):
        ...
        with raises(SchemaDefinitionMissingError, match='The labeler configuration must point to a schema definition.'):
            LabelerFactory.create('name', self.config, logger)
    ...
    def test_create_fails_when_schema_config_points_to_non_existing_file(self):
        ...
        with raises(InvalidSchemaDefinitionError, match='schema does not point to a schema definition file: Not a valid schema file: File not found: ".*".'):
            LabelerFactory.create('name', self.config, logger)
    ...
    def test_create_fails_when_include_parent_labels_is_not_boolean(self):
        ...
        with raises(InvalidIncludeParentsValueError, match='"include_parent_labels" must be either true or false.'):
            LabelerFactory.create('name', self.config, logger)
...
class TestLabeler:
    ...
    def test_cannot_set_schema_set_to_non_schema_object(self):
        for non_ruleset in [None, 'string', 42, [], {}, DummyInput([])]:
            with raises(InvalidLabelingSchemaError, match='Labeler \\(%s\\): Not a labeling schema: .*' % self.labeler_name):
    ...
    def test_process_adds_labels_to_event_with_umlauts(self):
        ...
        expected = {'äpplyrüle': 'nö',
                    'label': {
                        'räpörter': ['windöws']
                    }}

        rule_dict = {"filter": "äpplyrüle: nö",
    ...
    def test_add_rules_fails_if_path_points_to_file(self):
        ...
        with raises(NotARulesDirectoryError, match=r'\'%s\', \'Not a rule directory: ".*"\'.*' % self.labeler_name):
            self.labeler.add_rules_from_directory([path_to_schema])

    def test_cannot_add_rules_that_violate_labeling_scheme(self):
        self.labeler.set_labeling_scheme(self.emtpy_schema)

        with raises(RuleDoesNotConformToLabelingSchemaError, match='Labeler \\(%s\\): Invalid rule file ".*": Does not conform to labeling schema.' % self.labeler_name):
            self.labeler.add_rules_from_directory([path_to_single_rule])

    def test_fails_when_trying_to_load_invalid_rule_file(self):
        self.labeler.set_labeling_scheme(self.emtpy_schema)

        with raises(InvalidRuleFileError, match=r'\'%s\', \'Invalid rule file ".*".*.' % self.labeler_name):
    ...
    def test_process_matches_event_with_array_with_one_element(self):
        ...
        expected = {'field_with_array': ['im_inside_an_array'],
                    'label': {
                        'some_new_label': ['some_new_value']
                    }
                    }

        rule_dict = {"filter": "field_with_array: im_inside_an_array",
                     'label': {
                         'some_new_label': ['some_new_value']
                     },
    ...
    def test_process_matches_event_with_array_if_at_least_one_element_matches(self):
        ...
        expected = {'field_with_array': ['im_inside_an_array', 'im_also_inside_that_array!'],
                    'label': {
                        'some_new_label': ['some_new_value']
                    }
                    }
    ...
    def test_process_matches_event_with_array_with_one_element_with_regex(self):
        ...
        rule_dict = {"filter": "field_with_array: im_inside_an_a.*y",
                     "regex_fields": [
                         "field_with_array"
                     ],
                     'label': {
    ...
    def test_process_matches_event_with_array_with_one_element_with_regex_one_without(self):
        ...
        rule_dict = {"filter": "field_with_array: im_inside_.*y AND field_with_array: im_also_inside_that_array",
                     "regex_fields": [
                         "field_with_array"
                     ],
                     'label': {
