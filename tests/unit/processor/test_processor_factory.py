...
from logprep.processor.processor_factory_error import (InvalidConfigurationError, UnknownProcessorTypeError,
...
class TestProcessorFactory:
    def test_create_fails_for_an_empty_section(self):
        with raises(NotExactlyOneEntryInConfigurationError,
    ...
    def test_create_fails_if_config_is_not_an_object(self):
        with raises(InvalidConfigSpecificationError,
    ...
    def test_create_fails_for_unknown_type(self):
        for type_name in ['unknown', 'no such processor'] + [''.join(sample(ascii_letters, 6)) for i in range(5)]:
    ...
    def test_fails_when_section_contains_more_than_one_element(self):
        with raises(InvalidConfigurationError,
                    match='There must be exactly one processor definition per pipeline entry.'):
            ProcessorFactory.create({'first': {'type': 'donothing'},
                                     'second': {'type': 'donothing'}}, logger)
