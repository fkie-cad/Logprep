...
class TestGenericResolver:
    def test_resolve_generic_instantiates(self, temp_rule_dir, generic_resolver_config):
        ...
        generic_resolver = GenericResolverFactory.create(
            "test-generic-resolver", generic_resolver_config, logger
        )

        assert isinstance(generic_resolver, GenericResolver)

    def test_resolve_generic_existing_not_dotted_field_without_conflict_match(
    ...
    def test_resolve_generic_existing_not_dotted_field_without_conflict_no_match(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
    ...
    def test_resolve_generic_existing_dotted_src_field_without_conflict_match(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
    ...
    def test_resolve_generic_existing_dotted_src_field_without_conflict_match_from_file_group_mapping_does_not_exist(
        ...
        with pytest.raises(
            GenericResolverError,
            match=r"GenericResolver \(test-generic-resolver\)\: Mapping group is missing in mapping file pattern!",
        ):
            generic_resolver.process(document)

    def test_resolve_generic_match_from_file_and_file_does_not_exist(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
        ...
        with pytest.raises(
            GenericResolverError,
            match=r"GenericResolver \(test-generic-resolver\)\: Additions file \'foo\' not found!",
        ):
            generic_resolver.process(document)

    def test_resolve_generic_existing_dotted_src_field_without_conflict_no_match(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
    ...
    def test_resolve_generic_existing_dotted_dest_field_without_conflict_match(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
    ...
    def test_resolve_generic_existing_dotted_dest_field_without_conflict_no_match(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
    ...
    def test_resolve_generic_with_multiple_match_first_only(
        self, temp_rule_dir, generic_resolver_config
    ):
        rule = [
            {
