...
class TestClustererRule:
    def test_rules_are_equal(self, rule_definition):
        rule1 = ClustererRule(LuceneFilter.create(rule_definition['filter']),
                              rule_definition['clusterer'])

        rule2 = ClustererRule(LuceneFilter.create(rule_definition['filter']),
                              rule_definition['clusterer'])

        assert rule1 == rule2

    def test_rules_are_not_equal(self, rule_definition):
        rule = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        rule_diff_target = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        rule_diff_pattern = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        rule_diff_repl = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        rule_diff_filter = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])
    ...
    def test_rule_creation_from_dict(self, rule_definition):
        ...
        expceted_rule = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        assert rule_from_dict == expceted_rule

    def test_tests_always_list(self, rule_definition):
        rule_no_test = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'])

        rule_dict_test = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'], tests={'foo': 'bar'})

        rule_list_test = ClustererRule(LuceneFilter.create(
            rule_definition['filter']), rule_definition['clusterer'], tests=[{'foo': 'bar'}])
