...
class TestPseudonomyzerRule:
    def test_rules_are_equal(self, specific_rule_definition):
        rule1 = PseudonymizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                                  specific_rule_definition['pseudonymize'])

        rule2 = PseudonymizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                                  specific_rule_definition['pseudonymize'])

        assert rule1 == rule2

    def test_rules_are_not_equal(self, specific_rule_definition):
        rule = PseudonymizerRule(LuceneFilter.create(specific_rule_definition['filter']),
                                 specific_rule_definition['pseudonymize'])

        rule_diff_pseudo = PseudonymizerRule(LuceneFilter.create(
            specific_rule_definition['filter']), specific_rule_definition['pseudonymize'])

        rule_diff_filter = PseudonymizerRule(LuceneFilter.create(
            specific_rule_definition['filter']), specific_rule_definition['pseudonymize'])
