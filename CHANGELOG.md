...
* add new `decoder` processor to decode values from event field, starting with `json`, `base64`, `clf` (see: https://en.wikipedia.org/wiki/Common_Log_Format), `nginx` parser for kubernetes ingress, `syslog_rfc3164`, `syslog_rfc3164_local`, `syslog_rfc5324`, `logfmt`, `cri`, `docker`, `decolorize` (removing color codes in logs), `dict2list` (converting string mapping to list of strings)
